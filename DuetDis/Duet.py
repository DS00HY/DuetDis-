...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, in_planes=526):
        self.inplanes = 64
        self.planes = 128
        self.expansion = 4
...
def resnet46(pretrained=False,in_planes=526):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(BasicBlock, [3, 6, 10, 3], in_planes)
    if pretrained:
        pass
        #model.load_state_dict(model_zoo.load_url(model_urls['resnet18']))
    return model
def seresnet101(pretrained=False, in_planes=526):
    model = ResNet(Bottleneck, [3, 4, 23, 3], in_planes)
    if pretrained:
        pass
    return model

def seresnet152(pretrained=False,in_planes=526):
    model = ResNet(Bottleneck, [3, 8, 36, 3], in_planes)
    if pretrained:
        pass
    return model
